...
function Dashboard() {
	...
	const getDataDashboard = (dataToken) => {
		dashboardServices({ dispatch, setSnackbar, token: dataToken?.token });
	};
	useEffect(() => {
	...
	const handleUpdateRate = (dataToken) => {
		adminUpdateRateSV({
			token: dataToken?.token,
			id_rate: rate?.id || 1,
				...
				{data.map((item, index) => {
					return (
						<tr key={index} style={{ fontSize: '14px' }}>
							...
							<td className="item-w200 text-left">
								{item.payment.email}
							</td>
							<td className="text-left item-w150">
								{numberUtils.formatVND(item.Wallet.balance)}
							</td>
							<td className="text-left">
								<span
									className={`${
										item.payment.roles + 'bgc'
									} status`}
								>
									{item.payment.roles}
								</span>
		...
		<div className={`${cx('dashboard-container')}`}>
			...
			<div className={`${cx('chart-container')}`}>
				<div className={`${cx('chart-item-container')}`}>
					...
					<ChartItem
						title="Tổng tài sản"
						value={numberUtils.formatVND(totalBalance || 0)}
					/>
					<ChartItem
						title="Commission"
			...
			<div className={`${cx('general-table-container')}`}>
				...
				<div style={{ width: '100%', overflowX: 'auto' }}>
					...
					if (index + 1 >= start && index + 1 <= end) return true;
						...
						endPagiCus={end}
					>
						<RenderBodyTableUser data={dataUser?.filter((row, index) => {
					if (index + 1 >= start && index + 1 <= end) return true;
				})} />
					</TableData>
				</div>
