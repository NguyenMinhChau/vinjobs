...
function User() {
    ...
    const useDebounceUser = useDebounce(user, 3000);
    useEffect(() => {
    ...
    if (user) {
        dataUserFlag = dataUserFlag.filter((item) => {
            return (
                ...
                item.payment.rule.toLowerCase().includes(user.toLowerCase()) ||
    ...
    return (
        <>
            ...
            <General
                ...
                totalData={
                    dataUser?.total || dataUser?.data?.total
                        ? dataUser.total || dataUser?.data?.total
                        : dataUserFlag?.length
                }
                PaginationCus={!(dataUser?.total || dataUser?.data?.total)}
